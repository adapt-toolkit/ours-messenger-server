...
export default function FleetApp() { return <ConfigurationProvider><NotificationProvider><FleetAppContent /></NotificationProvider></ConfigurationProvider>; }
function FleetAppContent() {
  ...
  return <div className={'fleet-app' + (mobileDetail && displayPage.kind === 'home' && section !== 'tasks' ? ' fleet-in-chat' : '') + (displayPage.kind === 'account' ? ' fleet-account-mode' : '')}>
    ...
    <div className="fleet-body">
      ...
      {(page.kind === 'profile' || page.kind === 'contacts') && <DialogShell closeAdornment={<NotificationBadge node="root" />} title="Profile" wide onClose={closeProfile} className="fleet-modal fleet-profile-modal">{!modal && <ProfilePage page={page} agents={agents} go={go} back={backPage} openChat={openChat} openRoom={openRoom} modal={setModal} />}{dialogs}</DialogShell>}
