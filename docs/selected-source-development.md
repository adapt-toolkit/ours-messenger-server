...
Reproducible selected builds set `SOURCE_DATE_EPOCH` from the selected Messenger Git revision's committer timestamp. `version.json.time` records that source epoch; it is not the wallclock build time. The recipe rejects a conflicting caller value. A source-only checkout must provide the matching epoch together with its existing SHA/CLEAN provenance inputs. Ordinary `npm run build` retains wallclock metadata when the variable is absent. The epoch alone does not establish reproducibility: toolchain and dependency inputs must also match.
