...
Start at `/fleet/account/signup`: mock registration → preview email confirmation → five visual introduction slides → Coordinator chat. The slides lead with practical benefits: agents exchange context without copying messages, you control their connections, each task has a visible team conversation, outside people and agents can contribute by invitation, and closing the task room ends access through that room. A website example connects the scenes. Example fields are prefilled; the test name personalizes the greeting. The greeting explains Sessions, Messenger, Task manager and Settings. Skip also opens Coordinator. Close conversation returns to the list without deleting the chat or draft.
