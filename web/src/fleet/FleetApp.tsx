...
import { Menu, Plus, Moon, Sun, Settings, User, MoreHorizontal, Info } from 'lucide-react';
...
export default function FleetApp() {
  ...
  const activeAgent = agents.find(a => a.id === workChat);
  const selectedTask = page.kind === 'task' ? tasks.find(t => t.id === page.id) : undefined;
  ...
  const go = (next: Page) => { pageStack.current.push(pageRef.current); setPage(next); setModal(null); };
  ...
  return <div className="fleet-app">
    <header className="fleet-nav">
      ...
      <button className="icon-btn fleet-mobile-menu" aria-label="Navigate" onClick={() => setModal({ kind: 'navigation' })}><Menu size={20} /></button>
      <nav aria-label="Main navigation">{(['work', 'messenger', 'tasks'] as Section[]).map(s => <button key={s} className={'btn' + (s === section && page.kind === 'home' ? ' primary' : '')} onClick={() => switchSection(s)}>{s[0].toUpperCase() + s.slice(1)}</button>)}</nav>
    ...
    <div className="fleet-body">
      <div className={'fleet-workspace' + (mobileDetail ? ' fleet-show-detail' : '')} hidden={page.kind !== 'home' || section === 'tasks'}>
        <aside className="fleet-list" hidden={section !== 'work'}>
          <h1>Work</h1><div className="fleet-segment">{['Persistent', 'Temporary'].map(v => <button key={v} aria-pressed={v === lifetime} onClick={() => { setLifetime(v); setNested(null); }}>{v}</button>)}</div>
          {task ? <><Button onClick={() => setNested(null)}>‹ Temporary</Button><h2>{task.name}</h2><p className="muted">Task · {task.agents.length} agents · Workstation</p><Row title="Room" subtitle="Shared chat · 2 unread" avatar="#" onClick={() => openRoom(task.id)} end={<span className="fleet-badge">2</span>} />{task.agents.map(id => { const a = agents.find(a => a.id === id); return a && <Row key={id} title={a.name} subtitle={`Direct agent chat · ${a.state}`} onClick={() => openChat(id)} />; })}<Button onClick={() => setModal({ kind: 'add-agent', id: task.id })}>＋ Agent</Button><Button onClick={() => setModal({ kind: 'task-menu', id: task.id })}>Task ⋯</Button></> : <><SearchField value={search} onChange={setSearch} placeholder={lifetime === 'Persistent' ? 'Search agents' : 'Search sessions and tasks'} />{lifetime === 'Temporary' && <><div className="fleet-actions"><Button onClick={() => setModal({ kind: 'new-agent' })}>＋ Session</Button><Button onClick={() => setModal({ kind: 'new-task' })}>＋ Task</Button></div>{tasks.filter(t => matches(t.name)).map(t => <Row key={t.id} title={t.name} subtitle={`Task · ${t.agents.length} agents · ${t.status}`} onClick={() => openRoom(t.id, false)} />)}</>}{agents.filter(a => a.lifetime === lifetime && !a.taskId && matches(a.name)).map(a => <div key={a.id} className={a.id === workChat ? 'fleet-selected' : ''}><Row title={a.name} subtitle={`${a.lifetime} · ${a.state}`} onClick={() => openChat(a.id)} /></div>)}<p className="fleet-list-note">{lifetime === 'Persistent' ? 'Agents that stay configured' : 'Tasks and standalone temporary sessions'}</p></>}
        </aside>
        ...
        <section className="fleet-conversation" aria-label={section === 'messenger' ? 'Messenger' : 'My Agent chat'}>
          {section === 'work' && <div className="fleet-context"><div><small>{activeId?.startsWith('room-') ? 'SHARED ROOM · YOU → EVERYONE' : `DIRECT AGENT · YOU → ${activeAgent?.name.toUpperCase() ?? 'AGENT'}`}</small><span>{task?.name ?? activeAgent?.lifetime} · Workstation</span></div><div className="fleet-actions">{task && <Button onClick={() => go({ kind: 'task', id: task.id })}>View task</Button>}{activeId?.startsWith('room-') ? <><Button onClick={() => setModal({ kind: 'members', id: nested! })}>Members · {1 + (task?.agents.length ?? 0)}</Button><button className="icon-btn" aria-label="Room information" onClick={() => setModal({ kind: 'room-info' })}><Info size={18} /></button></> : <><button className="icon-btn" aria-label="Agent actions" onClick={() => setModal({ kind: 'agent-menu', id: activeAgent?.id })}><MoreHorizontal /></button>{task && <Button onClick={() => openRoom(task.id)}>Room <span className="fleet-badge">2</span></Button>}</>}</div></div>}
          {openedChats.map(id => { const a = agents.find(a => a.id === id || `messenger-${a.id}` === id); const c = contacts.find(c => c.id === id) ?? contact(id, id.startsWith('room-') ? 'Room' : a?.name ?? 'Agent'); return <div key={id} className="fleet-chat-slot signal-stage show-detail" hidden={activeId !== id}><Conversation contact={c} messages={histories[id] ?? []} onBack={() => setMobileDetail(false)} onOpenContact={() => id.startsWith('room-') ? setModal({ kind: 'members', id: id.slice(5) }) : go({ kind: 'profile', id })} onSend={(text, reply) => send(id, text, reply)} onSendFile={async () => { notify('Attachment selected. File delivery is unavailable in this preview.'); }} /></div>; })}
          {section === 'work' && workChat === 'coordinator' && <div className="fleet-chat-context"><Row title="Launch website" subtitle="Task created · pair · 2 agents" onClick={() => openRoom(initialTasks[0].id, false)} /><Button onClick={() => go({ kind: 'settings', editor: 'template' })}>Configuration</Button><Button onClick={() => go({ kind: 'contacts', id: 'coordinator' })}>Agent identity connections</Button></div>}
          {section === 'work' && activeAgent?.role === 'Developer' && <div className="fleet-tool"><details><summary>✓ Read files · 3 <span>Completed · Expand output</span></summary><p>Read index.html, styles.css and app.tsx. Heading hierarchy and form labels checked.</p><Button onClick={() => setModal({ kind: 'tool-output' })}>Open full output</Button></details><div className="fleet-permission"><span>Test command · {permission}<small>npm test · /home/you/work/website</small></span>{permission === 'Awaiting permission' && <><Button onClick={() => setPermission('Denied')}>Deny</Button><Button primary onClick={() => setPermission('Allowed once · Complete')}>Allow once</Button></>}<Button onClick={() => { setPermission('Stopped'); notify('Response stopped'); }}>Stop response</Button></div></div>}
        </section>
      </div>
      {page.kind === 'home' && section === 'tasks' && <main className="fleet-page fleet-board-page"><PageHeader title="Tasks" actions={<><select aria-label="Filter task list" value={listFilter} onChange={e => setListFilter(e.target.value)}>{['All lists', ...lists].map(l => <option key={l}>{l}</option>)}</select><Button onClick={() => setModal({ kind: 'lists' })}>Manage lists</Button><Button primary onClick={() => setModal({ kind: 'new-task' })}>＋ New task</Button></>} /><SearchField value={taskSearch} onChange={setTaskSearch} placeholder="Search tasks" /><div className="fleet-board">{statuses.map(status => { const rows = tasks.filter(t => t.status === status && (listFilter === 'All lists' || t.list === listFilter) && t.name.toLowerCase().includes(taskSearch.toLowerCase())); return <section key={status} className="fleet-column"><h2>{status} <span>{rows.length}</span></h2>{rows.map(t => <button key={t.id} className="fleet-task-card" onClick={() => go({ kind: 'task', id: t.id })}><strong>{t.name}</strong><span>{t.list}</span><small className={t.blocked ? 'fleet-warning' : ''}>{t.blocked ? `Blocked · ${t.blocked}` : t.status === 'Provisioning' ? 'Joining room · 2/3' : t.status === 'Backlog' ? 'Not started' : t.status === 'Failed' ? 'Launch failed' : t.status === 'Done' ? 'Completed today' : 'Developer + Critic'}</small></button>)}</section>; })}</div></main>}
      {page.kind === 'task' && (selectedTask ? <main className="fleet-page"><PageHeader title={selectedTask.name} subtitle={`TASK · ${selectedTask.id}`} onBack={() => switchSection('tasks')} actions={<><Button onClick={() => setModal({ kind: 'task-menu', id: selectedTask.id })}>{selectedTask.status} ▾</Button><Button primary onClick={() => { updateTask(selectedTask.id, { status: selectedTask.status === 'Backlog' ? 'Provisioning' : 'Review' }); if(selectedTask.status === 'Backlog') setModal({ kind: 'provisioning', id: selectedTask.id }); }}>{selectedTask.status === 'Backlog' ? 'Start task' : 'Send to review →'}</Button></>} /><div className="fleet-task-detail"><section><h2>Description</h2><p>{selectedTask.description}</p>{selectedTask.blocked && <p className="fleet-warning">Blocked · {selectedTask.blocked}</p>}<h2>Work <span className="muted">{selectedTask.agents.length} agents</span></h2><Row title={`${selectedTask.name} · Room`} subtitle={selectedTask.closed ? 'Closed' : 'Shared room · Ready'} avatar="#" onClick={() => openRoom(selectedTask.id)} />{selectedTask.agents.map(id => { const a = agents.find(a => a.id === id); return a && <div key={id} className="fleet-member"><Row title={a.name} subtitle={a.state} onClick={() => openChat(a.id)} /><Button onClick={() => setModal({ kind: 'remove-agent', id: a.id })}>Remove</Button></div>; })}{!selectedTask.agents.length && <p>Add your first temporary task agent.</p>}<Button onClick={() => setModal({ kind: 'add-agent', id: selectedTask.id })}>＋ Agent</Button></section><aside><h2>Details</h2><Row title="List" subtitle={selectedTask.list} onClick={() => setModal({ kind: 'move-task', id: selectedTask.id })} /><Row title="Template" subtitle="Pair · v1" /><Row title="Created" subtitle="Today, 10:24" /><Row title="Updated" subtitle="2 minutes ago" /><p className="muted">Created from Work</p></aside></div></main> : <main className="fleet-page"><PageHeader title="Task no longer available" onBack={() => switchSection('tasks')} /></main>)}
      {(page.kind === 'profile' || page.kind === 'contacts') && <ProfilePage page={page} agents={agents} go={go} back={backPage} openChat={openChat} openRoom={openRoom} modal={setModal} />}
      {page.kind === 'settings' && <SettingsPage editor={page.editor} go={go} notify={notify} />}
      {page.kind === 'account' && <AccountPage step={page.step} go={go} notify={notify} />}
      {page.kind === 'empty' && <main className="fleet-form-page"><PageHeader title="Your first agent session" subtitle="Start a focused conversation with an agent on Workstation." onBack={() => go({ kind: 'home' })} /><Row title="One agent" subtitle="Choose a template and host folder" onClick={() => setModal({ kind: 'new-agent' })} /><Row title="A task with several agents" subtitle="Create a task session with a shared room" onClick={() => setModal({ kind: 'new-task' })} /><Row title="Persistent agents" subtitle="Configured agents appear here when available" onClick={() => switchSection('work')} /><Row title="Looking for someone?" subtitle="Open Messenger" onClick={() => switchSection('messenger')} /></main>}
    </div>
    {modal && <FleetDialogs modal={modal} close={() => setModal(null)} setModal={setModal} agents={agents} setAgents={setAgents} tasks={tasks} setTasks={setTasks} lists={lists} setLists={setLists} taskId={contextTaskId} openChat={openChat} openRoom={openRoom} go={go} switchSection={switchSection} notify={notify} updateTask={updateTask} />}
