#!/usr/bin/env bash
#
# EPHEMERAL nightly versioning for the single-package messenger-server, mirroring ours-mcp's
...
# the messenger has no lockstep cycle: we bump the PATCH so nightlies track the messenger's own
...
case "$LEVEL" in patch|minor) ;; *) echo "invalid nightly bump level" >&2; exit 1;; esac
...
nightly_max_index() { # <pkg-name> <base>
  npm view "$1" versions --json 2>/dev/null | node -e '
    let s=""; process.stdin.on("data",d=>s+=d).on("end",()=>{
      let a = JSON.parse(s);
      if (!Array.isArray(a) && typeof a !== "string") throw new Error("invalid registry versions");
...
pub_v="$(npm view "$NAME" version)"   # @latest only; nightlies never raise the base
[[ "$local_v" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ && "$pub_v" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || { echo "invalid stable base version" >&2; exit 1; }
