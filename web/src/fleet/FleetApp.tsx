...
import { Menu, Plus, Moon, Sun, Settings, User, MoreHorizontal, Info, X } from 'lucide-react';
...
export default function FleetApp() {
  ...
  return <div className="fleet-app">
    <header className="fleet-nav" hidden={displayPage.kind === 'account'}>
      ...
      <nav aria-label="Main navigation">{(['work', 'messenger', 'tasks'] as Section[]).map(s => <button key={s} className={'btn' + (s === section && displayPage.kind === 'home' ? ' primary' : '')} onClick={() => switchSection(s)}>{s[0].toUpperCase() + s.slice(1)}</button>)}</nav>
    ...
    <div className="fleet-body">
      <div className={'fleet-workspace' + (mobileDetail ? ' fleet-show-detail' : '')} hidden={displayPage.kind !== 'home' || section === 'tasks'}>
        ...
        <section className="fleet-conversation" aria-label={section === 'messenger' ? 'Messenger' : 'My Agent chat'}>
          {section === 'work' && activeId && <div className="fleet-context"><div><small>{activeId?.startsWith('room-') ? 'SHARED ROOM · YOU → EVERYONE' : `DIRECT AGENT · YOU → ${activeAgent?.name.toUpperCase() ?? 'AGENT'}`}</small><span>{task?.name ?? activeAgent?.lifetime} · Workstation</span></div><div className="fleet-actions"><button className="icon-btn" aria-label="Close conversation" onClick={() => { setWorkChat(null); setMobileDetail(false); }}><X size={18} /></button>{task && <Button onClick={() => go({ kind: 'task', id: task.id })}>View task</Button>}{activeId?.startsWith('room-') ? <><Button onClick={() => setModal({ kind: 'members', id: nested! })}>Members · {1 + (task?.agents.length ?? 0)}</Button><button className="icon-btn" aria-label="Room information" onClick={() => setModal({ kind: 'room-info' })}><Info size={18} /></button></> : <><button className="icon-btn" aria-label="Agent actions" onClick={() => setModal({ kind: 'agent-menu', id: activeAgent?.id })}><MoreHorizontal /></button>{task && <Button onClick={() => openRoom(task.id)}>Room <span className="fleet-badge">2</span></Button>}</>}</div></div>}
      ...
      {displayPage.kind === 'settings' && <SettingsPage editor={displayPage.editor} go={go} notify={notify} />}
