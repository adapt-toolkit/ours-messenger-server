...
export function FleetDialogs(p: Props) {
  ...
  switch(m.kind) {
    ...
    case 'task-menu': title = task?.name ?? 'Task actions'; body = <>{[['move-task', 'Move to list'], ['block-task', 'Block task'], ['finish-task', 'Finish & close room'], ['close-task', 'Close task'], ['cancel-task', 'Cancel task'], ['delete-task', 'Delete task']].map(([kind, title]) => <Row key={kind} title={title} onClick={() => next(kind)} />)}{task?.status === 'Provisioning' && <Row title="View provisioning" onClick={() => next('provisioning')} />}<Row title="View task" onClick={() => p.go({ kind: 'task', id: m.id! })} /></>; break;
