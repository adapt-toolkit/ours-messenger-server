...
export default function FleetApp() {
  ...
  return <div className={'fleet-app' + (mobileDetail && displayPage.kind === 'home' && section !== 'tasks' ? ' fleet-in-chat' : '') + (displayPage.kind === 'account' ? ' fleet-account-mode' : '')}>
    <header className="fleet-nav" hidden={displayPage.kind === 'account'}>
      ...
      <IconButton className=" fleet-nav-trigger" aria-label="Navigate" onClick={() => setModal({ kind: 'navigation' })}><LauncherIcon /></IconButton>
    ...
    <div className="fleet-body">
      <div className={'fleet-workspace' + (mobileDetail ? ' fleet-show-detail' : '')} hidden={displayPage.kind !== 'home' || section === 'tasks'}>
        ...
        <section className="fleet-conversation" aria-label={section === 'messenger' ? 'Messenger' : 'My Agent chat'}>
          ...
          {openedChats.map(id => { const a = agents.find(a => a.id === id || `messenger-${a.id}` === id); const c = contacts.find(c => c.id === id) ?? contact(id, id.startsWith('room-') ? 'Room' : a?.name ?? (draftAgent?.id === id ? 'New chat' : 'Agent')); return <div key={id} className="fleet-chat-slot signal-stage show-detail" hidden={activeId !== id}><Conversation headerActions={<div className="fleet-header-actions">{section === 'work' && (a || id.startsWith('room-')) && <IconButton className="" aria-label={id.startsWith('room-') ? 'Room actions' : 'Agent actions'} onClick={() => setModal({ kind: id.startsWith('room-') ? 'members' : 'agent-menu', id: id.startsWith('room-') ? id.slice(5) : id })}><MoreHorizontal size={20} /></IconButton>}<IconButton className=" fleet-desktop-control" aria-label="Close conversation" onClick={() => { if(section === 'messenger') setMessengerChat(null); else setWorkChat(null); setMobileDetail(false); }}><X size={18} /></IconButton></div>} timelineFooter={draftAgent?.id === id ? <ChatSetup draft={a?.brain ? { ...a, brain: a.brain, permissions: a.permissions! } : draftAgent} locked={!!a} onChange={next => { if(!startedDrafts.current.has(id)) setDraftAgent(next); }} /> : a?.brain ? <ChatSetup draft={{ ...a, brain: a.brain, permissions: a.permissions! }} locked /> : id === 'coordinator' && !coordinatorWelcomed ? <div className="fleet-chat-context"><Row title="Launch website" subtitle="Task created · pair · 2 agents" onClick={() => openRoom(initialTasks[0].id, false)} /><Button onClick={() => go({ kind: 'settings', editor: 'template' })}>Configuration</Button><Button onClick={() => go({ kind: 'contacts', id: 'coordinator' })}>Agent identity connections</Button></div> : a?.id === id && a.role === 'Developer' ? <AgentActivity onOpenOutput={() => setModal({ kind: 'tool-output' })} /> : undefined} contact={c} messages={histories[id] ?? []} onBack={() => setMobileDetail(false)} onOpenContact={draftAgent?.id === id && !a ? undefined : () => id.startsWith('room-') ? setModal({ kind: 'members', id: id.slice(5) }) : go({ kind: 'profile', id })} onSend={(text, reply) => send(id, text, reply)} onSendFile={async () => { notify('Attachment selected. File delivery is unavailable in this preview.'); }} /></div>; })}
        </section>
      </div>
      ...
      {(page.kind === 'profile' || page.kind === 'contacts') && <DialogShell title="Profile" wide onClose={closeProfile} className="fleet-modal fleet-profile-modal">{!modal && <ProfilePage page={page} agents={agents} go={go} back={backPage} openChat={openChat} openRoom={openRoom} modal={setModal} />}{dialogs}</DialogShell>}
