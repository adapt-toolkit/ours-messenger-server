...
export function SessionList(p: Props) {
  ...
  const row = (id: string, name: string, last: string, onClick: () => void, unread = 0) => <ContactRow key={id} c={{ ...contact(id, name), last, when: '', unread }} active={p.selected === id} onClick={onClick} />;
  ...
  return <LayoutGroup id="fleet-sessions"><div className="listcol">
    <div className="listcol-head">
      <div className="conversation-list-modes" role="tablist" aria-label="Session lifetime" data-mode={p.lifetime === 'Temporary' ? 'identity' : 'recent'}>
        {choices.map((value, index) => <button key={value} role="tab" id={`session-tab-${value}`} aria-controls="session-list-panel" aria-selected={value === p.lifetime} tabIndex={value === p.lifetime ? 0 : -1} className={value === p.lifetime ? 'active' : ''} onClick={() => p.setLifetime(value)} onKeyDown={event => {
    ...
    <div className="listcol-scroll" id="session-list-panel" role="tabpanel" aria-labelledby={`session-tab-${p.lifetime}`}>
      {p.task ? <>
        <div className="fleet-session-context"><Button onClick={p.leaveTask}>‹ Temporary</Button><h2>{p.task.name}</h2><Button onClick={() => p.taskMenu(p.task!.id)}>Task ⋯</Button></div>
        <div className="conversation-group">
          {matches('Room') && row(`room-${p.task.id}`, 'Room', 'Shared chat', () => p.openRoom(p.task!.id), 2)}
        ...
        {p.lifetime === 'Temporary' && <><div className="group-label">Tasks</div><div className="conversation-group">{p.tasks.filter(t => matches(t.name)).map(t => row(t.id, t.name, `Task · ${t.agents.length} agents · ${t.status}`, () => p.openRoom(t.id, false)))}</div><div className="group-label">Chats</div></>}
