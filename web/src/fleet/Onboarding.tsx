...
export function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  return <main className="fleet-intro">
    ...
    <div className="fleet-intro-main"><Scene step={step} /><section className="fleet-intro-copy"><p className="fleet-intro-eyebrow">{slide.eyebrow}</p><h1 tabIndex={-1} ref={heading}>{slide.title}</h1><p>{slide.body}</p>{step === 1 && <p className="fleet-intro-security"><Lock size={16} aria-hidden /><span><strong>You talk. We handle the security.</strong>Modern cryptography protects your connections, verifies who’s speaking, and keeps messages private and intact.</span></p>}</section></div>
