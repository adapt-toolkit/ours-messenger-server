...
import {
  HTML_PREVIEW_ROOT_STYLE, HTML_PREVIEW_ROOT_STYLE_MARKER,
...
<style>:root{--ink:#15213a}body{display:grid;color:var(--ink)}</style><div data-ours-preview-root>fake</div><script src=/leak>evil()</script></head><body onload=evil()>
...
<math><a href="#math" onmouseover=evil()>math</a></math><style DATA-OURS-PREVIEW-ROOT>body{display:none}</style><h2 id=section style="color:red">Привет</h2></body></html>`;
...
const tree = parse(transformed.toString('utf8')); const nodes = [tree]; const allNodes = []; const tags = []; const attrs = [];
while (nodes.length) { const node = nodes.pop(); allNodes.push(node); if (node.tagName) tags.push(node.tagName); if (node.attrs) attrs.push(...node.attrs.map((attr) => ({ tag: node.tagName, name: attr.name, prefix: attr.prefix, value: attr.value }))); if (node.content) nodes.push(node.content); if (node.childNodes) nodes.push(...node.childNodes); }
...
const marked = allNodes.filter((node) => node.attrs?.some((attr) => attr.name === HTML_PREVIEW_ROOT_STYLE_MARKER));
assert.equal(marked.length, 1, 'hostile duplicate markers are replaced by one trusted node');
assert.equal(marked[0].tagName, 'style');
assert.equal(marked[0].childNodes?.[0]?.value, HTML_PREVIEW_ROOT_STYLE);
const head = allNodes.find((node) => node.tagName === 'head');
assert.equal(head.childNodes.at(-1), marked[0], 'trusted root invariant is the final HEAD child');

const synthesized = parse(transformHtmlPreview(Buffer.from('<p>missing head</p>')).toString());
const synthesizedHtml = synthesized.childNodes.find((node) => node.tagName === 'html');
const synthesizedHead = synthesizedHtml.childNodes.find((node) => node.tagName === 'head');
assert.equal(synthesizedHead.childNodes.at(-1).attrs[0].name, HTML_PREVIEW_ROOT_STYLE_MARKER, 'parse5-synthesized HEAD receives the invariant');
